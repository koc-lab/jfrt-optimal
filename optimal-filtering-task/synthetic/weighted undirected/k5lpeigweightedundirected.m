...
for z = 5
  ...
  for i = 1:N
     for k = 1:K
          distance = norm(node(i,:)-node(I(i,k),:))^2;
  ...
  % figure
  % plot(ar,MSE(1,:),'-.','LineWidth',2); hold on;
  % plot(ar,MSE(2,:),'--','LineWidth',2);
  % plot(ar,MSE(3,:),'LineWidth',2);
  % legend('\sigma = 0.5', '\sigma = 1', '\sigma = 1.5');
  %
  %  xlabel('$a$','fontsize',14,'interpreter','latex');
  %  ylabel('MSE');
  % grid on;
  % save("weightedundirectedJFRTMSE.mat","MSE")
  save(['fixednodelp.mat'],'MSE')
end
close all;
